...
create_superuser() {
    if [ ! -z "$DJANGO_SUPERUSER_EMAIL" ] && [ ! -z "$DJANGO_SUPERUSER_USERNAME" ] && [ ! -z "$DJANGO_SUPERUSER_PASSWORD" ]; then
        echo "Creating superuser..."
        
        # Create the superuser
        python manage.py createsuperuser --noinput --email "$DJANGO_SUPERUSER_PHONE" --username "${DJANGO_SUPERUSER_USERNAME}" &>/dev/null
